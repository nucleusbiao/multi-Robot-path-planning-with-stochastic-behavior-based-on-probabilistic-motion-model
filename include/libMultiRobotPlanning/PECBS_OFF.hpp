...
#include "a_star_epsilon_old.hpp"
...
/*! \brief
Probability based Conflict-Based-Search (PCBS) to solve the\n
multi-Robot path planning with stochastic behavior based on probabilistic motion model.
Details of the algorithm can be found in the following paper:\n
Biao Hu, Haonan Wang, Zhengcai Cao:\n
"bability-based Path Planning for Multi-Robot Systems with Stochastic Behavior in a Grid Map".\n
2021 IEEE SMC. 2310-2315(2021).https://doi.org/10.1109/SMC52423.2021.9659070

Enhanced Conflict-Based-Search (ECBS) algorithm to solve the
Multi-Agent Path-Finding (MAPF) problem within a given suboptimality bound
...
\tparam EdgeCost A look-up table to record CostTable. To reduce repeated calculation.
\tparam CostTable Agents' custom cost for each edge.
...
constraints
  ...
  - `Cost focalStateHeuristic(const State& s1,const State& s2, int gScore, const
...
*/
template <typename State, typename Action,typename EdgeCost,typename CostTable,typename Cost, typename Conflict,
          typename Constraints, typename Environment>
class ECBS {
public:
    ECBS(Environment& environment, double w) : m_env(environment), m_w(w) {}
    ...
    bool search(const std::vector<State>& initialStates,
        ...
        for (size_t i = 0; i < initialStates.size(); ++i) {
            if (i < solution.size() && solution[i].states.size() > 1) {
                ...
                LowLevelEnvironment llenv(m_env, i, start.constraints[i],
                                          start.solution,edgeCost);
                LowLevelSearch_t lowLevel(llenv, m_w);
                bool success = lowLevel.search(initialStates[i], start.solution[i],edgeCost);
                /*if(i==7){
                    std::cout<<"???:"<<std::endl;
                }*/
                if (!success) {
        ...
        start.focalHeuristic = m_env.focalHeuristic(start.solution,edgeCost);
        ...
        while (!open.empty()) {
            ...
            Conflict conflict;
            //std::cout<<1<<std::endl;
            if (!m_env.getFirstConflict(P.solution, conflict,edgeCost)) {
            ...
            std::map<size_t, Constraints> constraints;
            m_env.createConstraintsFromConflict(conflict, constraints,P.solution);
            for (const auto& c : constraints) {
                ...
                LowLevelEnvironment llenv(m_env, i, newNode.constraints[i],
                                          newNode.solution,edgeCost);
                LowLevelSearch_t lowLevel(llenv, m_w);
                bool success = lowLevel.search(initialStates[i], newNode.solution[i],edgeCost);

                newNode.cost += newNode.solution[i].cost;
                newNode.LB += newNode.solution[i].fmin;
                newNode.focalHeuristic = m_env.focalHeuristic(newNode.solution,edgeCost);
                std::cout<<"focalHeuristic: "<<newNode.focalHeuristic<<std::endl;
                if (success) {
    ...
    struct LowLevelEnvironment {
        LowLevelEnvironment(
            ...
            : m_env(env),
              ...
              m_solution(solution),edgeCost(edgeCost){
        ...
        Cost focalStateHeuristic(const State& s, Cost gScore,EdgeCost& edgeCost) {
            return m_env.focalStateHeuristic(s, gScore, m_solution,edgeCost);
        }

        Cost focalTransitionHeuristic(const State& s1, const State& s2,
                                      Cost gScoreS1, Cost gScoreS2,EdgeCost& edgeCost) {
        ...
        void getNeighbors(const State& s,
                          ...
                          EdgeCost& edgeCost) {
            m_env.getNeighbors(s, neighbors,edgeCost);
        }
...
private:
    Environment& m_env;
    double m_w;
    typedef AStarEpsilon<State, Action,EdgeCost, CostTable,Cost, LowLevelEnvironment>
    LowLevelSearch_t;
};
