...
/*!
  \example PCBS_OFF.cpp Example that solves the Multi-Agent Path-Finding (MAPF)
...
Conflict-Based-Search (CBS) algorithm to solve the Multi-Agent
...
public:
    ...
    bool search(const std::vector<State>& initialStates,
        ...
        for (size_t i = 0; i < initialStates.size(); ++i) {
            ...
            // } else {
            LowLevelEnvironment llenv(m_env, i, start.constraints[i],edgeCost);
        ...
        while (!open.empty()) {
            ...
            std::map<size_t, Constraints> constraints;
            // printf("%d,%d,%d,%f,%f\n",i++,conflict.agent1,conflict.agent2,conflict.agent1Time,conflict.agent2Time);
            ...
            for (const auto& c : constraints) {
                ...
                LowLevelEnvironment llenv(m_env, i, newNode.constraints[i],edgeCost);
                LowLevelSearch_t lowLevel(llenv);
...
private:
    ...
    struct LowLevelEnvironment {
        ...
        LowLevelEnvironment(Environment& env, size_t agentIdx,
                            const Constraints& constraints,EdgeCost& edgeCost)
            : m_env(env),edgeCost(edgeCost)
            // , m_agentIdx(agentIdx)
...
private:
    Environment& m_env;
    typedef AStar<State, Action,EdgeCost,CostTable, Cost, LowLevelEnvironment> LowLevelSearch_t;
};
