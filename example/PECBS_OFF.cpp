...
#include <libMultiRobotPlanning/PECBS_OFF.hpp>
#include "timer.hpp"
...
double time_div=1; //wait time
double t_Div=0.02; //time step
double threshold=0.6;//time threshold
double a=0.9;//triangular probability distribution's parameter a
double c=1.1;//triangular probability distribution's parameter c
double probThreshold=0.05;//probability threshold
double velocity=1;//velocity
double FCost=1;//forward cost
double TCost=1.2;//turn cost
double BCost=1.5;//backward cost
...
std::ostream& operator<<(std::ostream& os, const CombineAction& a) {
    switch(a){
    case CombineAction::Wait:
        os << "Wait";
        break;
    case CombineAction::Turn:
        os << "Turn";
        break;
    case CombineAction::Forward:
        os << "Forward";
        break;
    case CombineAction::Backward:
        os << "Backward";
        break;
    }
    return os;
...
struct CostTable{
    ...
    CostTable(size_t agentID,size_t step,size_t waitTime,size_t forwardTime,size_t turnTime,size_t backwardTime):
        agentID(agentID),step(step),waitTime(waitTime),forwardTime(forwardTime),turnTime(turnTime),backwardTime(backwardTime){
        if(step==1){
            double cost=FCost*forwardTime+TCost*turnTime+BCost*backwardTime;
            triangleProbFuc(pdf, a*cost,cost,c*cost, t_Div,cost*2);
        }else if(step==0){
            pdf.push_back(1/t_Div);
        }
    }
    ...
    size_t step=0;
    size_t waitTime=0;
    size_t forwardTime=0;
    size_t turnTime=0;
    size_t backwardTime=0;
    ...
    bool operator==(const CostTable& other) const {
        if(agentID==other.agentID&&waitTime==other.waitTime&&forwardTime==other.forwardTime&&
                turnTime==other.turnTime&&backwardTime==other.backwardTime){
            return true;
        }
        else{
            return false;
...
struct hash<CostTable> {
    size_t operator()(const CostTable& s) const {
        ...
        boost::hash_combine(seed, s.forwardTime);
        boost::hash_combine(seed, s.turnTime);
        boost::hash_combine(seed, s.backwardTime);
        return seed;
    }
};
}  // namespace std
...
struct State {
    State(Action lateAction,double time, int x, int y, size_t stepNum,
          size_t waitTime,size_t forwardTime,size_t turnTime,size_t backwardTime)
        :lateAction(lateAction),time(time), x(x), y(y),stepNum(stepNum),
          waitTime(waitTime),forwardTime(forwardTime),turnTime(turnTime),backwardTime(backwardTime){
    }
...
public:
        ...
        for (size_t i = 0; i < solution.size(); ++i) {
            if (i != m_agentIdx && !solution[i].states.empty()) {
                for(size_t j=0;j<=solution[i].states.size()-1;j++){
                    State state2 = getState(i, solution, j);
                    if(s.equal(state2)){
                        if (s.stepNum==(solution[m_agentIdx].states.size() - 1)&&s.time<=state2.time){
                            ++numConflicts;
                            break;
                        }
                        if (state2.stepNum==(solution[i].states.size() - 1)&&state2.time<=s.time){
                            ++numConflicts;
                            break;
                        }
                        if(std::abs(s.time-state2.time)<thresholdCost){
                            ++numConflicts;
                            break;
                            /*double prob=0;
                            auto s1pdf=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime));
                            auto s2pdf=edgeCost.costTable.find(CostTable(i,state2.stepNum,state2.waitTime));
    ...
    int focalTransitionHeuristic(
            const State& s1a, const State& s1b, double /*gScoreS1a*/, double /*gScoreS1b*/,
            const std::vector<PlanResult<State, Action, double> >& solution,
            EdgeCost& edgeCost) {
        int numConflicts = 0;
        double tDiv=t_Div;
        for (size_t i = 0; i < solution.size(); ++i) {
            if (i != m_agentIdx && !solution[i].states.empty()) {
                for(size_t j=0;j<solution[i].states.size()-1;j++){
                    State s2a=getState(i,solution,j);
                    State s2b=getState(i,solution,j+1);
                    if (s1a.equal(s2b)&&
                            s1b.equal(s2a)){
                        if((s1a.time<=s2b.time&&s1a.time>=s2a.time)||(s2a.time>=s1a.time&&s2a.time<=s1b.time)){
                            ++numConflicts;
                            break;
                        }

                        /*if(s1a.time-s2b.time>3||s2a.time-s1b.time>3) continue;
                        double prob=0;
                        auto s1apdf=edgeCost.costTable.find(CostTable(m_agentIdx,s1a.stepNum,s1a.waitTime));
                        auto s1bpdf=edgeCost.costTable.find(CostTable(m_agentIdx,s1b.stepNum,s1b.waitTime));
                        auto s2apdf=edgeCost.costTable.find(CostTable(i,s2a.stepNum,s2a.waitTime));
                        auto s2bpdf=edgeCost.costTable.find(CostTable(i,s2b.stepNum,s2b.waitTime));
                        edgeProbCal(prob,s1apdf->pdf,s1bpdf->pdf,s2apdf->pdf,s2bpdf->pdf,tDiv);
                        if(s1a.stepNum==0&&s2a.stepNum==0){prob=1;};
                        if(prob>probThreshold){
                            ++numConflicts;
                            break;
                        }*/
    ...
    // Count all conflicts
    int focalHeuristic(const std::vector<PlanResult<State, Action, double> >& solution,
                       EdgeCost& edgeCost) {
        int numConflicts = 0;
                ...
                for(size_t stepNum1 = 1; stepNum1 <= solution[i].states.size() - 1;stepNum1++)
                {
                    for(size_t stepNum2 =1; stepNum2 <=solution[j].states.size() - 1;stepNum2++)
                    {
                        ...
                        if (state1.equal(state2)) {
                            //std::cout<<"find 1: ("<<i<<","<<j<<")"<<std::endl;
                            if (state1.stepNum==(solution[i].states.size() - 1)&&state1.time<=state2.time){
                                ++numConflicts;
                                //std::cout<<"VC 1: ("<<i<<","<<j<<")"<<std::endl;
                                break;
                            }
                            /*if(i==6&&j==7){
                                std::cout<<"time"<<state1.time<<" , "<<state2.time<<std::endl;
                                for(auto ac: solution[j].states){
                                    std::cout<<ac.first.time<<std::endl;
                                }
                            }*/
                            if (state2.stepNum==(solution[j].states.size() - 1)&&state2.time<=state1.time){
                                //std::cout<<"VC 2: ("<<i<<","<<j<<")"<<std::endl;
                                ++numConflicts;
                                break;
                            }

                            if(std::abs(state1.time-state2.time)<=10*thresholdCost) {
                            double prob=0;
                            auto s1pdf=edgeCost.costTable.find(CostTable(i,state1.stepNum,state1.waitTime,state1.forwardTime,state1.turnTime,state1.backwardTime));
                            auto s2pdf=edgeCost.costTable.find(CostTable(j,state2.stepNum,state2.waitTime,state2.forwardTime,state2.turnTime,state2.backwardTime));
                            vertexProbCal(prob,s1pdf->pdf,s2pdf->pdf,tDiv,thresholdCost);
                            if(prob>probThreshold){
                                //std::cout<<"VC 3: ("<<i<<","<<j<<")"<<std::endl;
                                ++numConflicts;
                                break;
                            }
                            }
                ...
                for(size_t stepNum1 = 0; stepNum1 < solution[i].states.size() - 1;stepNum1++)
                {
                    for(size_t stepNum2 = 0; stepNum2 < solution[j].states.size() - 1;stepNum2++)
                    {
                        ...
                        if (state1a.equal(state2b)&&
                            ...
                            double prob=0;
                            auto s1apdf=edgeCost.costTable.find(CostTable(i,state1a.stepNum,state1a.waitTime,state1a.forwardTime,state1a.turnTime,state1a.backwardTime));
                            auto s1bpdf=edgeCost.costTable.find(CostTable(i,state1b.stepNum,state1b.waitTime,state1b.forwardTime,state1b.turnTime,state1b.backwardTime));
                            auto s2apdf=edgeCost.costTable.find(CostTable(j,state2a.stepNum,state2a.waitTime,state2a.forwardTime,state2a.turnTime,state2a.backwardTime));
                            auto s2bpdf=edgeCost.costTable.find(CostTable(j,state2b.stepNum,state2b.waitTime,state2b.forwardTime,state2b.turnTime,state2b.backwardTime));
                            edgeProbCal(prob,s1apdf->pdf,s1bpdf->pdf,s2apdf->pdf,s2bpdf->pdf,tDiv);
        ...
        //std::cout<<"numConflicts: "<<numConflicts<<std::endl;
        return numConflicts;
    }

    bool isSolution(const State& s) {
        /*if(m_agentIdx==7){
            std::cout<<"zuobiao; ("<<s.x<<","<<s.y<<" )"<<std::endl;
            if(s.x == m_goals[m_agentIdx].x && s.y == m_goals[m_agentIdx].y){
                std::cout<<"yueshu  "<<m_lastGoalConstraint<<std::endl;
    ...
    void getNeighbors(const State& s,
                      ...
                      EdgeCost& edgeCost) {
        neighbors.clear();
        auto ecWaitfind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime+1,s.forwardTime,s.turnTime,s.backwardTime));
        auto ecForwardfind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime,s.forwardTime+1,s.turnTime,s.backwardTime));
        auto ecTurnfind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime+1,s.backwardTime));
        auto ecBackwardfind=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime+1));
        {
            if(ecWaitfind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime));
                bool sign=(ec!=edgeCost.costTable.end());
                CostTable ecWait(m_agentIdx,ec->step+1,ec->waitTime+1,s.forwardTime,s.turnTime,s.backwardTime);
                for(double i=0;i<=time_div;i+=t_Div){
                    ecWait.pdf.emplace_back(0);

                }
                for(size_t i=0;i<ec->pdf.size();i++){
                    ecWait.pdf.emplace_back(ec->pdf[i]);
                }
                edgeCost.costTable.emplace(ecWait);
            }
            State n(Action::Wait,s.time + time_div, s.x, s.y,s.stepNum+1,s.waitTime+1,s.forwardTime,s.turnTime,s.backwardTime);
            if (stateValid(n) && transitionValid(s, n)) {neighbors.emplace_back(Neighbor<State, Action, double>(n, Action::Wait, time_div));}
        }
        {
            if(ecForwardfind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime));
                CostTable ecForward(m_agentIdx,ec->step+1,ec->waitTime,s.forwardTime+1,s.turnTime,s.backwardTime);
                auto forwardPdf=edgeCost.costTable.find(CostTable(m_agentIdx,2,0,1,0,0));
                convdx(ecForward.pdf,ec->pdf,forwardPdf->pdf);
                edgeCost.costTable.emplace(ecForward);
            }
            if(ecTurnfind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime));
                CostTable ecTurn(m_agentIdx,ec->step+1,ec->waitTime,s.forwardTime,s.turnTime+1,s.backwardTime);
                auto turnPdf=edgeCost.costTable.find(CostTable(m_agentIdx,2,0,0,1,0));
                convdx(ecTurn.pdf,ec->pdf,turnPdf->pdf);
                edgeCost.costTable.emplace(ecTurn);
            }
            if(ecBackwardfind!=edgeCost.costTable.end()){

            }else{
                auto ec=edgeCost.costTable.find(CostTable(m_agentIdx,s.stepNum,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime));
                CostTable ecBackward(m_agentIdx,ec->step+1,ec->waitTime,s.forwardTime,s.turnTime,s.backwardTime+1);
                auto backwardPdf=edgeCost.costTable.find(CostTable(m_agentIdx,2,0,0,0,1));
                convdx(ecBackward.pdf,ec->pdf,backwardPdf->pdf);
                edgeCost.costTable.emplace(ecBackward);
            }
            {
                State n(Action::Left,s.time, s.x - 1, s.y,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime);
                switch(s.lateAction){
                case Action::Left:n.time+=FCost;n.forwardTime++;break;
                case Action::Right:n.time+=BCost;n.backwardTime++;break;
                case Action::Up:n.time+=TCost;n.turnTime++;break;
                case Action::Down:n.time+=TCost;n.turnTime++;break;
                case Action::Wait:n.time+=FCost;n.forwardTime++;break;
                }
                if (stateValid(n) && transitionValid(s, n)) {neighbors.emplace_back(Neighbor<State, Action, double>(n, Action::Left, n.time-s.time));}
            }
            {
                State n(Action::Right,s.time, s.x + 1, s.y,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime);
                switch(s.lateAction){
                case Action::Left:n.time+=BCost;n.backwardTime++;break;
                case Action::Right:n.time+=FCost;n.forwardTime++;break;
                case Action::Up:n.time+=TCost;n.turnTime++;break;
                case Action::Down:n.time+=TCost;n.turnTime++;break;
                case Action::Wait:n.time+=FCost;n.forwardTime++;break;
                }
                if (stateValid(n) && transitionValid(s, n)) {neighbors.emplace_back(Neighbor<State, Action, double>(n, Action::Right,n.time-s.time));}
            }
            {
                State n(Action::Up,s.time, s.x, s.y+1,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime);
                switch(s.lateAction){
                case Action::Left:n.time+=TCost;n.turnTime++;break;
                case Action::Right:n.time+=TCost;n.turnTime++;break;
                case Action::Up:n.time+=FCost;n.forwardTime++;break;
                case Action::Down:n.time+=BCost;n.backwardTime++;break;
                case Action::Wait:n.time+=FCost;n.forwardTime++;break;
                }
                if (stateValid(n) && transitionValid(s, n)) {neighbors.emplace_back(Neighbor<State, Action, double>(n, Action::Up, n.time-s.time));}
            }
            {
                State n(Action::Down,s.time, s.x, s.y-1,s.stepNum+1,s.waitTime,s.forwardTime,s.turnTime,s.backwardTime);
                switch(s.lateAction){
                case Action::Left:n.time+=TCost;n.turnTime++;break;
                case Action::Right:n.time+=TCost;n.turnTime++;break;
                case Action::Up:n.time+=BCost;n.backwardTime++;break;
                case Action::Down:n.time+=FCost;n.forwardTime++;break;
                case Action::Wait:n.time+=FCost;n.forwardTime++;break;
                }
                if (stateValid(n) && transitionValid(s, n)) { neighbors.emplace_back(Neighbor<State, Action, double>(n, Action::Down, n.time-s.time));}
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()("help", "produce help message")(
                ...
                "input file (YAML)")("output,o",
                                     ...
                                     "output file (YAML)")(
                "suboptimality,w", po::value<double>(&w)->default_value(1.3),
                "suboptimality bound");
    ...
    for (const auto& node : config["agents"]) {
        ...
        startStates.emplace_back(State(Action::Wait,0, start[0].as<int>(), start[1].as<int>(),0,0,0,0,0));
    ...
    ECBS<State, Action,EdgeCost,CostTable, double, Conflict, Constraints, Environment> cbs(mapf, w);

    std::vector<PlanResult<State, Action, double> > solution;

    EdgeCost edgeCost;
    for(size_t agentID=0;agentID<startStates.size();agentID++){
        edgeCost.costTable.insert(CostTable(agentID,0,0,0,0,0));
        edgeCost.costTable.insert(CostTable(agentID,1,0,1,0,0));
        edgeCost.costTable.insert(CostTable(agentID,1,0,0,1,0));
        edgeCost.costTable.insert(CostTable(agentID,1,0,0,0,1));
    }
